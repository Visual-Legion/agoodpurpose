	...
	$(() => {
		...
		(function() {
			...
			function scrollArray(elem, left, top) {
				...
				var isWindowScroll =
					elem === scrollRoot || elem === document.body;

				// if we haven't already fixed the behavior,
				// and it needs fixing for this sesh
				if (
					elem.$scrollBehavior == null &&
					isScrollBehaviorSmooth(elem)
				) {
			...
			function wheel(event) {
				...
				if (isMac) {
					if (
						event.wheelDeltaX &&
						isDivisible(event.wheelDeltaX, 120)
					) {
						deltaX =
							-120 *
			...
			function keydown(event) {
				...
				switch (event.keyCode) {
					...
					case key.end:
						var scroll =
							overflowing.scrollHeight - overflowing.scrollTop;
			...
			function isNodeName(el, tag) {
				return (
					el &&
					(el.nodeName || "").toLowerCase() === tag.toLowerCase()
				);
			}
			...
			var isOldSafari =
				isSafari &&
				(/Version\/8/i.test(userAgent) ||
					/Version\/9/i.test(userAgent));
